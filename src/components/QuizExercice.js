...
export default function QuizExercice({ exercice, onComplete, onBadgeEarned }) {
  ...
  async function enregistrerResultat(correct) {
    ...
    await supabase.from('resultats').insert({
      ...
      reponse_donnee: selected || reponseLibre || (exercice.type === 'vrai_faux' ? selected : ''),
      correct: correct,
    ...
    // Toujours appeler onComplete avec le résultat
    if (onComplete) {
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        <Text style={styles.typeBadge}>
          {exercice.type === 'qcm' ? 'QCM' : exercice.type === 'vrai_faux' ? 'Vrai/Faux' : 'Reponse libre'}
        </Text>
        ...
        <View style={[styles.resultat, isCorrect ? styles.resultatCorrect : styles.resultatIncorrect]}>
          ...
          <View>
            ...
            <Text style={styles.resultatCorrection}>
              Reponse : {exercice.reponse_correcte}
            </Text>
          </View>
        </View>
      )}
...
const styles = StyleSheet.create({
  card: { backgroundColor: '#FFFFFF', borderRadius: 16, padding: 20, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  typeBadge: { fontSize: 12, fontWeight: 'bold', color: '#2563EB', backgroundColor: '#DBEAFE', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, overflow: 'hidden' },
  points: { fontSize: 12, color: '#64748B', fontWeight: '600' },
  question: { fontSize: 16, fontWeight: '600', color: '#1E293B', lineHeight: 24, marginBottom: 20 },
  optionsGrid: { gap: 8 },
  optionButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderWidth: 2, borderColor: '#E2E8F0', borderRadius: 12, padding: 14, marginBottom: 8 },
  optionCorrect: { backgroundColor: '#DCFCE7', borderColor: '#16A34A' },
  optionIncorrect: { backgroundColor: '#FEE2E2', borderColor: '#DC2626' },
  optionKey: { fontSize: 14, fontWeight: 'bold', color: '#2563EB', backgroundColor: '#DBEAFE', width: 30, height: 30, borderRadius: 15, textAlign: 'center', lineHeight: 30, marginRight: 12, overflow: 'hidden' },
  optionKeyCorrect: { color: '#16A34A', backgroundColor: '#DCFCE7' },
  optionKeyIncorrect: { color: '#DC2626', backgroundColor: '#FEE2E2' },
  optionText: { fontSize: 14, color: '#334155', flex: 1 },
  vfRow: { flexDirection: 'row', gap: 12 },
  vfButton: { flex: 1, backgroundColor: '#F8FAFC', borderWidth: 2, borderColor: '#E2E8F0', borderRadius: 12, padding: 16, alignItems: 'center' },
  vfText: { fontSize: 16, fontWeight: 'bold' },
  textInput: { backgroundColor: '#F8FAFC', borderWidth: 2, borderColor: '#E2E8F0', borderRadius: 12, padding: 14, fontSize: 15, minHeight: 60, textAlignVertical: 'top' },
  submitButton: { backgroundColor: '#2563EB', borderRadius: 10, padding: 14, alignItems: 'center', marginTop: 10 },
  submitText: { color: '#FFFFFF', fontSize: 15, fontWeight: '600' },
  resultat: { flexDirection: 'row', alignItems: 'center', marginTop: 16, padding: 14, borderRadius: 12, gap: 12 },
  resultatCorrect: { backgroundColor: '#DCFCE7' },
  resultatIncorrect: { backgroundColor: '#FEE2E2' },
  resultatEmoji: { fontSize: 28 },
  resultatText: { fontSize: 15, fontWeight: 'bold', color: '#1E293B' },
  resultatCorrection: { fontSize: 13, color: '#64748B', marginTop: 2 },
});